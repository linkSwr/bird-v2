...
function upload2PlantForm(req, res, reslove, reject, chunk, reqChunk) {
    ...
    // 只上报JSON数据
    if (platformUrl.hostname && IS_JSON && !~urlsHasReport.indexOf(req.url)) {
        var request = http.request(options, function (res) {
            ...
            res.on('end', function () {
                // console.info('report end: ', req.url);
        ...
        if (req.method === 'POST') {
            result.requestData = JSON.parse((reqChunk || new Buffer(0)).toString('utf8'));
        }
...
module.exports = function (_config) {
    ...
    return {
        type: 'response',
        processer: platformUrl.href ? upload2PlantForm : function (req, res, reslove) {reslove()}
    };
};
