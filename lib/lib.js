...
function entry(req, res, next) {
    ...
    requsetTasks(req, res).then(function () {
        return standardPromise = standardTasks(req, res);
    }).catch(function (data) {
        // console.info('finally', arguments[0]);
        // console.info('resetRes chunk', standardPromise.reason());
        responseTasks(req, res, next, data.chunk, data.reqChunk);
    });
}
...
function standardTasks(req, res) {
    ...
    .then(resetRes).catch(function (err) {
        resetRes();
        // 将数据返回给下一个流程使用
 ...
 */
function responseTasks(req, res, next, chunk, reqChunk) {
    var _end = res.end;
    var _send = res.send;
    var _json = res.json;
    // var IS_REJECT = false;
    ...
    return new Promise(function (resolve, reject) {
        ...
        middleTasksManager.execute('response', req, res, resolve, reject, chunk, reqChunk);
...
/**
 * @method getJsFileWithAP
 *
 * @param {string} dir
 *
 * @return {Array}
 *
 * @description: 获取指定路径的下的所有JS文件，返回所有文件的绝对路径地址
 */
function getJsFileWithAP(dir) {
    var result;
    result = fs.readdirSync(dir);
    result.forEach(function (el, index, array) {
        if (/\.js$/.test(el)) {
            array[index] = path.join(dir, el);
        }
        else {
            array.splice(index, 1);
            array[index] && (array[index] = path.join(dir, array[index]));
        }
    });
    return result;
}
...
module.exports = function (_config, _middleTasksManager, _useAuth) {
    ...
    var fileName;
    var taskDir = path.join(process.cwd(), _config.tasksDir);
    var protoTask = [];
    config = _config;
    ...
    // 读取task文件列表
    if (_config.tasksDir) {
        taskList = getJsFileWithAP(taskDir);
    }
    protoTask = getJsFileWithAP(__dirname + '/middletasks');
    taskList = taskList.concat(protoTask);
    taskList.forEach(function (el, index, array) {
        fileName = el.replace(/([^\/\\]*)\.js$/, '$1');
        // task = require('./middletasks/' + fileName);
        task = require(el.replace(/.js$/, ''));
