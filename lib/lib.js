 ...
 *
 * @param {Array} args end, end, json的调用参数
 *
 * @description: 修改response的标准 end, end, json 方法， 让流程跑到最后再结束response
 */
function setResAction(res, resolve, args) {
    ...
    taskList.forEach(function (el, index, array) {
        ...
        try {
            ...
            if (!~taskType.indexOf(task.type)) {
                console.error(new Error(el, 'task type neither "request" nor "response"'));
            }
        }
    });
    return entry;
